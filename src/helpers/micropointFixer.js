...
export async function processMicropointStock(xmlData) {
  ...
  const categorizedProducts = await applyCategories(promotedProductsHandled, 'Micropoint')
  const commonFieldMapping = mapToCommonFields(categorizedProducts)
